...
def test_run_cycle_skips_when_policy_blocked():
    ...
    with patch(
        ...
        "stellarhydra.graph.hydra_graph.get_settings",
        return_value=__import__("stellarhydra.config", fromlist=["Settings"]).Settings(
            hydra_max_drip_xlm_per_hour=10
        ),
    ):
        result = run_cycle(thread_id="policy-block")
