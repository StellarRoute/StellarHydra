...
def test_run_cycle_skips_when_policy_blocked():
    ...
    assert result.action_plan is not None
    assert result.action_plan.stream_amount_xlm == 10.0
    assert result.action_plan.policy_ok is True


def test_run_cycle_skips_when_kill_switch_active():
    mock_signal = RoutingSignal(
        base="native",
        quote="USDC",
        amount="100",
    ...
    with patch(
        ...
        result = run_cycle(
            thread_id="kill-switch-skip",
            settings=Settings.model_construct(
                hydra_max_drip_xlm_per_hour=1000,
                drips_dry_run=True,
                config_path=Path("/nonexistent/settings.yaml"),
            ),
        )
    ...
    assert any("kill switch" in e.lower() for e in result.errors)
